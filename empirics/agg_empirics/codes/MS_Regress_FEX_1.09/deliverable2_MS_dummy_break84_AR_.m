...
data = 100*[diff(match_sales) diff(match_GDP) share_rCIPI_pot(2:end)];
...
S{1}=zeros(1,2*(1+lags*nvar)+1);S{1}(1,1:2)=ones(1,2);S{1}(1,end)=1;        % Defining which parts of the equation will switch states (column 1 and variance only)
S{2}=zeros(1,2*(1+lags*nvar)+1);S{2}(1,1:2)=ones(1,2);S{2}(1,end)=1;     % Defining which parts of the equation will switch states (column 1 and variance only)
S{3}=zeros(1,2*(1+lags*nvar)+1);S{3}(1,1:2)=ones(1,2);S{3}(1,end)=1;      % Defining which parts of the equation will switch states (column 1 and variance only)
