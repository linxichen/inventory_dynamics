...
match_GDP = rGDP./GDPPOT;
match_sales = rSales./GDPPOT;
data_prestd = [match_sales match_GDP share_rCIPI_pot];
data_diff = diff(data_prestd);
data = (data_diff-mean(data_diff))./std(data_diff);

% transformations, multiply share cipi by 10, then everything by 1e4
% data(:,3) = 10*data(:,3);
% data = 1e4*data;
...
S{1}=ones(1,2+lags*nvar+1);S{1}(1,end)=1;        % Defining which parts of the equation will switch states (column 1 and variance only)
S{2}=ones(1,2+lags*nvar+1);S{2}(1,end)=1;     % Defining which parts of the equation will switch states (column 1 and variance only)
S{3}=ones(1,2+lags*nvar+1);S{3}(1,end)=1;      % Defining which parts of the equation will switch states (column 1 and variance only)
...
safe_dates = date_serial(start+1+lags:end-shift,:);
figure
plot(safe_dates,Spec_Out.smoothProb(:,3));
h = gca;
datetick('x','yyyy','keepticks')
xlabel('Time');
ylabel('Smoothed States Probabilities');
