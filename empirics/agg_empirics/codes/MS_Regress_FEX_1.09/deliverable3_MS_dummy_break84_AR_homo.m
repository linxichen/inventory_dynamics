...
match_GDP = ln_rGDP;
match_sales = ln_rSales;
data = 100*[diff(match_sales) diff(match_GDP) diff(share_rCIPI_pot)];
...
S{1}=zeros(1,2+lags*nvar+1);S{1}(1,1:end-1)=ones(1,2+lags*nvar);S{1}(1,end)=1;        % Defining which parts of the equation will switch states (column 1 and variance only)
S{2}=zeros(1,2+lags*nvar+1);S{2}(1,1:end-1)=ones(1,2+lags*nvar);S{2}(1,end)=1;     % Defining which parts of the equation will switch states (column 1 and variance only)
S{3}=zeros(1,2+lags*nvar+1);S{3}(1,1:end-1)=ones(1,2+lags*nvar);S{3}(1,end)=1;      % Defining which parts of the equation will switch states (column 1 and variance only)
